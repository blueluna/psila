...
where
    Cipher: BlockCipher,
{
    pub fn new(cipher: Cipher, mode: Mode, initialisation_vector: [u8; BLOCK_SIZE]) -> Self {
        Self {
            cipher,
            mode,
...
impl CryptoBackend for OpenSslBackend {
    fn ccmstar_decrypt(
        ...
        let mut cipher = Aes128Cbc::new(
            OpenSslCipher::new_aes_128_ecb(Mode::Encrypt),
            Mode::Encrypt,
            [0u8; BLOCK_SIZE],
        );
        cipher.set_key(key)?;
...
mod tests {
    ...
    fn test_aes_128_ecb_decrypt() {
        ...
        for (input, output) in message
            .chunks_exact(BLOCK_SIZE)
            .zip(decrypted.chunks_exact_mut(BLOCK_SIZE))
        {
            cipher.process_block(input, output).unwrap();
        }
        for (calculated, correct) in decrypted
            .chunks_exact(BLOCK_SIZE)
            .zip(result.chunks_exact(BLOCK_SIZE))
        {
            assert_eq!(calculated, correct);
    ...
    fn test_aes_128_cbc_decrypt() {
        ...
        let mut decrypted = [0u8; 256];
        let mut cipher = Aes128Cbc::new(
            OpenSslCipher::new_aes_128_ecb(Mode::Decrypt),
            Mode::Decrypt,
            initialisation_vector,
        );
        cipher.set_key(&key).unwrap();
